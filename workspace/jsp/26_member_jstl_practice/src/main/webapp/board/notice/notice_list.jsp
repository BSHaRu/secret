...
<c:if test="${rs.rowCount > 0}">
	...
	<c:forEach var="notice" items="${rs.rows }">
		<jsp:useBean id="vo" class="vo.NoticeVO" /> <%-- 영역객체에 useBean이 없으면 생성하는거임 --%>
		...
		<c:remove var="vo" /> <%-- 영역객체에서 지워줘야 새로운 vo를 추가해줄 수 있음 --%>
	</c:forEach>
</c:if>
				...
				<tr>
					<th colspan="4">
						...
						<c:forEach var="i" begin="${pm.startPage }" end="${pm.endPage }">
							<c:choose>
								<c:when test="${pm.cri.page eq i }"> <%-- 현재페이지 일경우는 링크가 필요없음 --%>
